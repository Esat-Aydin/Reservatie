...
namespace Reservation
{
    public abstract class Reserveren
    {
        ...
        public void ReserveringBeheren()
        {
            ...
            // Inladen Json Module 
            var MyFilmsData = new WebClient().DownloadString(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\Filmsdata.json");
            string myJsonString = new WebClient().DownloadString(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\snacksdrinks.json");
            string myUserData = new WebClient().DownloadString(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\SampleLog.json");
            ...
            int Index = -1;
            if (Reservatie_code == "0")
            {
                Scherm.Screens.ReturnToPreviousScreen("VisitorScreen");
            }
            ConsoleCommands.Textkleur("wit");
...
/*                Console.Clear();
                    ...
                    if (DateInFutureCheck(TestDateTime) == true)
                    {
                        ...
                        while (CorrectInput == false)
                        {
                            ...
                            if (Int32.TryParse(Film_keuze, out IntFilmKeuze))
                            {
                                ...
                                Console.Write("\n\nToets ["); ConsoleCommands.Textkleur("zwart"); Console.Write("4"); ConsoleCommands.Textkleur("wit"); Console.Write("] Terug gaan\n");
                                CorrectInput = true;
                                int intTijdKeuze = 0;
                                ...
                                while (CorrectInput2 == false)
                                {
                                    ...
                                    {
                                        if (intTijdKeuze > 4 || intTijdKeuze <= 0)
                                        {
                                            ConsoleCommands.Textkleur("wit"); Console.WriteLine("_____________________________________________________________________________________________\n");
        ...
        public void DatumChecker(string Chosen_film, List<string> Show_films, Dictionary<string, string[]> Show_tijden)
        {
            ...
            for (int i = 0; i < Show_films.Count + 1; i++)
            {
                ...
                if (Chosen_film == (film_showw))
                {
                    ...
                    ConsoleCommands.Textkleur("wit");
                    Console.WriteLine("Voer uw gewenste datum in (DD/MM/YYYY): ");
                    ConsoleCommands.Textkleur("wit"); Console.WriteLine("_____________________________________________________________________________________________\n");
                    ConsoleCommands.Textkleur("zwart");
                    string Chosen_date = Console.ReadLine();

                    
                    if (Chosen_date.Length != 10 || IsDateUserInputInteger(Chosen_date)== false)
                    {
                        ConsoleCommands.Textkleur("wit"); Console.WriteLine("_____________________________________________________________________________________________\n");
                        Console.WriteLine($"Dit is niet het goede format {Chosen_date}.\nProbeer het opnieuw met dit format: (DD/MM/YYYY)");
                        Thread.Sleep(3000);
                        Console.Clear();
                        DatumChecker(Chosen_film, Show_films, Show_tijden);
                    }
                    var UserDateTime = DateTimeReturner(Chosen_date);
                    if (DateInFutureCheck(UserDateTime) == true)
                    {
                        continue;
                    }
                    else
                    ...
                    if (DynamicFilmData[i - 1]["FilmDays"][Dagvdweek].Count > 0)
                    {
                        ...
                        {
                            ConsoleCommands.Textkleur("wit"); Console.Write("Toets ["); ConsoleCommands.Textkleur("zwart"); Console.Write(Count1); ConsoleCommands.Textkleur("wit"); Console.Write($"] voor {DynamicFilmData[i - 1]["FilmDays"][Dagvdweek][x]}\n");
                            Count1++;
                        }
                        ConsoleCommands.Textkleur("wit"); Console.WriteLine("_____________________________________________________________________________________________\n");
                        ConsoleCommands.Textkleur("zwart"); Console.Write("0"); ConsoleCommands.Textkleur("wit"); Console.Write("] Terug gaan\n");
                        ...
                        {
                            Console.Write($"U heeft de verkeerde input gegeven, probeer het opnieuw.");
                            Thread.Sleep(3000);
                            Console.Clear();
                            DatumChecker(Chosen_film, Show_films, Show_tijden);
                        }
                    }
                    else if (DynamicFilmData[i - 1]["FilmDays"][Dagvdweek].Count == 0)
                    {
                        Console.WriteLine($"Op {Chosen_date} draait de geselecteerde film niet.\nProbeer het opnieuw. ");
                        Thread.Sleep(3000);
                        Console.Clear();
                        DatumChecker(Chosen_film, Show_films, Show_tijden);
                    }
        ...
        public void ReserveerCodeMail(string Gezochte_Film, string Show_Tijden, string FilmDatum = null) //[FILMDATUM NIET AF] Deze method regelt de reservering en mailt het vervolgens naar de gebruiker - Callen: Gebruiker.ReserveerCodeMail();
        {
            static void ReservationToJSon(Gebruiker.Gebruiker Klant, string GeneratedCode)
            {
                ...
                var DataUser = File.ReadAllText(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\SampleLog.json"); //PATH VERANDEREN NAAR JOUW EIGEN BESTANDSLOCATIE ALS JE HIER EEN ERROR KRIJGT
                ...
                File.WriteAllText(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\SampleLog.json", DataUser);
            }
    ...
    "CinemaReservation"
        ...
        public void ReserveringAnnuleren(int Index, dynamic DynamicUserData)
        {
            ...
            if(Optie == "1")
            {
                ...
                File.WriteAllText(@"C:\Users\Edward\source\repos\Esat-Aydin\Reservatie\Reservatie\SampleLog.json", UserData);
